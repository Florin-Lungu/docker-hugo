...
prepare: src/bin/buildx
	...
	@sed -i.bak "s:DOCKER_CLI_EXPERIMENTAL=enabled docker buildx:buildx:g" target/bundle/Makefile && rm target/bundle/Makefile.bak
#	@sed -i.bak 's:--progress plain \\:--progress plain \\\n                --annotation $(DOCKER_METADATA_OUTPUT_ANNOTATIONS) \\:g' target/bundle/Makefile
	@sed -i.bak "s:--push:--provenance=true --sbom=true --push:g" target/bundle/Makefile && rm target/bundle/Makefile.bak
